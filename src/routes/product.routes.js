...
productRouter.get("/", async (req, res) => {
  ...
  try {
    ...
    if (query.search) {
      ...
      let data = await productModel.find({ type: query.type }, null, {
        sort: { updatedAt: -1 },
      });
      return res.status(201).send(data);
    } else if (query.collection) {
      let data = await productModel.find({ collect: query.collection }, null, {
        sort: { updatedAt: -1 },
      });
      return res.status(201).send(data);
    } else {
  ...
  try {
    if (token) {
      ...
      return res
        ...
        .send({
          ...data.toObject(),
          liked: checkliked(userFound.cart, data._id),
        });
    }
    let data = await productModel.findById(id);
    if (data) {
